# This Makefile is for the SingelCellSAM extension to perl.
#
...
#     ABSTRACT => q[Split and add 10x cell info from/to bam/sam files.]
#     AUTHOR => [q[Stefan Lang (Stefan.Lang@Med.Lu.SE)]]
#     BUILD_REQUIRES => {  }
#     CONFIGURE_REQUIRES => {  }
#     EXE_FILES => [q[bin/split10xsam.pl], q[bin/changeReadGroup.pl], q[bin/add10xTags.pl]]
#     INSTALLDIRS => q[site]
#     NAME => q[SingelCellSAM]
#     PREREQ_PM => { Test::Simple=>q[0.44] }
#     TEST_REQUIRES => {  }
#     VERSION_FROM => q[lib/SingelCellSAM.pm]
...
MAKE = make
FULLEXT = SingelCellSAM
BASEEXT = SingelCellSAM
PARENT_NAME = 
DLBASE = $(BASEEXT)
...
DISTNAME = SingelCellSAM
DISTVNAME = SingelCellSAM-0.01
...
EXE_FILES = bin/add10xTags.pl bin/changeReadGroup.pl bin/split10xsam.pl

pure_all :: $(INST_SCRIPT)/add10xTags.pl $(INST_SCRIPT)/changeReadGroup.pl $(INST_SCRIPT)/split10xsam.pl
	$(NOECHO) $(NOOP)

realclean ::
	$(RM_F) \
	  $(INST_SCRIPT)/add10xTags.pl $(INST_SCRIPT)/changeReadGroup.pl \
	  $(INST_SCRIPT)/split10xsam.pl 

$(INST_SCRIPT)/add10xTags.pl : bin/add10xTags.pl $(FIRST_MAKEFILE) $(INST_SCRIPT)$(DFSEP).exists $(INST_BIN)$(DFSEP).exists
	$(NOECHO) $(RM_F) $(INST_SCRIPT)/add10xTags.pl
	$(CP) bin/add10xTags.pl $(INST_SCRIPT)/add10xTags.pl
	$(FIXIN) $(INST_SCRIPT)/add10xTags.pl
	-$(NOECHO) $(CHMOD) $(PERM_RWX) $(INST_SCRIPT)/add10xTags.pl

$(INST_SCRIPT)/changeReadGroup.pl : bin/changeReadGroup.pl $(FIRST_MAKEFILE) $(INST_SCRIPT)$(DFSEP).exists $(INST_BIN)$(DFSEP).exists
	$(NOECHO) $(RM_F) $(INST_SCRIPT)/changeReadGroup.pl
	$(CP) bin/changeReadGroup.pl $(INST_SCRIPT)/changeReadGroup.pl
	$(FIXIN) $(INST_SCRIPT)/changeReadGroup.pl
	-$(NOECHO) $(CHMOD) $(PERM_RWX) $(INST_SCRIPT)/changeReadGroup.pl

$(INST_SCRIPT)/split10xsam.pl : bin/split10xsam.pl $(FIRST_MAKEFILE) $(INST_SCRIPT)$(DFSEP).exists $(INST_BIN)$(DFSEP).exists
	$(NOECHO) $(RM_F) $(INST_SCRIPT)/split10xsam.pl
	$(CP) bin/split10xsam.pl $(INST_SCRIPT)/split10xsam.pl
	$(FIXIN) $(INST_SCRIPT)/split10xsam.pl
	-$(NOECHO) $(CHMOD) $(PERM_RWX) $(INST_SCRIPT)/split10xsam.pl
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'abstract: '\''Split and add 10x cell info from/to bam/sam files.'\''' >> META_new.yml
	$(NOECHO) $(ECHO) 'author:' >> META_new.yml
	$(NOECHO) $(ECHO) '  - '\''Stefan Lang (Stefan.Lang@Med.Lu.SE)'\''' >> META_new.yml
	...
	$(NOECHO) $(ECHO) 'name: SingelCellSAM' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '   "abstract" : "Split and add 10x cell info from/to bam/sam files.",' >> META_new.json
	$(NOECHO) $(ECHO) '   "author" : [' >> META_new.json
	$(NOECHO) $(ECHO) '      "Stefan Lang (Stefan.Lang@Med.Lu.SE)"' >> META_new.json
	$(NOECHO) $(ECHO) '   ],' >> META_new.json
	...
	$(NOECHO) $(ECHO) '   },' >> META_new.json
	$(NOECHO) $(ECHO) '   "name" : "SingelCellSAM",' >> META_new.json
...
ppd :
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="SingelCellSAM" VERSION="0.01">' > SingelCellSAM.ppd
	$(NOECHO) $(ECHO) '    <ABSTRACT>Split and add 10x cell info from/to bam/sam files.</ABSTRACT>' >> SingelCellSAM.ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Stefan Lang (Stefan.Lang@Med.Lu.SE)</AUTHOR>' >> SingelCellSAM.ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> SingelCellSAM.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Test::Simple" VERSION="0.44" />' >> SingelCellSAM.ppd
	$(NOECHO) $(ECHO) '        <ARCHITECTURE NAME="x86_64-linux-gnu-thread-multi-5.30" />' >> SingelCellSAM.ppd
	$(NOECHO) $(ECHO) '        <CODEBASE HREF="" />' >> SingelCellSAM.ppd
	$(NOECHO) $(ECHO) '    </IMPLEMENTATION>' >> SingelCellSAM.ppd
	$(NOECHO) $(ECHO) '</SOFTPKG>' >> SingelCellSAM.ppd
